...
const ContextMenuOverlay = () => {
  ...
  const handleEditNode = () => {
    if (contextMenu.target.type === 'node') {
      selectComponent(contextMenu.target.componentId);
      openValueEditor(contextMenu.target.componentId);
      const component = useCircuitStore.getState().components.find(
        (item) => item.id === contextMenu.target.componentId
      );
      if (component) {
